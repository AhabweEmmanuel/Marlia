...
public class EmbedBottomSheet extends BottomSheet {
    ...
    public static void show(Context context, String title, String description, String originalUrl, final String url) {
        if (instance != null) {
            instance.destroy();
        }
        new EmbedBottomSheet(context, title, description, originalUrl, url).show();
    }
    ...
    @SuppressLint("SetJavaScriptEnabled")
    private EmbedBottomSheet(Context context, String title, String description, String originalUrl, final String url) {
        ...
        containerLayout = new FrameLayout(context) {
            ...
            protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
                int parentWidth = MeasureSpec.getSize(widthMeasureSpec);
                float scale = width / (float) parentWidth;
                int h = height;//(int) Math.min(height / scale, AndroidUtilities.displaySize.y / 2);
                super.onMeasure(widthMeasureSpec, MeasureSpec.makeMeasureSpec(h + AndroidUtilities.dp(48 + 36 + (hasDescription ? 22 : 0)) + 1, MeasureSpec.EXACTLY));
            }
        };
//        containerLayout.setOnTouchListener((v, event) -> true);
    ...
    protected boolean canDismissWithSwipe() {
        return false;//videoView.getVisibility() != View.VISIBLE || !videoView.isInFullscreen();
    }
